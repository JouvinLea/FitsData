...
#./make_false_data.py  "ash_stereo_thsq64" "Prod15_4_stereo"
            ...
            for (izen, zen) in enumerate(Observation.zenMC):
                for (igam, gamma) in enumerate(Observation.gammaMC):
                    for (iaz, az) in enumerate(Observation.azMC):
                        ...
                        outdir=indir+"/gamma_"+str(gamma)+"_nevents_"+str(n_events_to_draw)+"_time"+str(livetime.value)
                        outdir2=indir+"/gamma_"+str(gamma)+"_nevents_"+str(n_events_to_draw)+"_time"+str(livetime.value)+"/"+str(Observation.folder(eff,run_number))
                        try:
                            os.makedirs(outdir2)
                        except Exception:
                            pass
